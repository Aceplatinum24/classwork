...
# # In this first example, you'll see that the "turtle" '
# # (the little triangle on the screen) moved forward, turned left, and moved 
# # forward again.

# # Now, let's make it turn right. Also, we can set the shape to be an actual turtle 
...
#     # "Defining a function doesn’t cause the computer to do anything. 
#     # It is like giving a recipe to the computer. Give someone a recipe 
#     # for banana bread and they know how to make it. They haven’t actually made it
#     #  yet,they just know how. You have to tell them to make banana bread. 
#     # That is, after we define the function we must call the function before the code
#     # in it runs. To call a function, type the function name and follow it by 
#     # parenthesis. Do not use def, that is only used when 
#     # we define what the function does, not when we tell the computer to run it."
...
# # Our drawTriangle function works well, but what if we wanted to be able to draw 
# # different size triangles?

# # We could define drawBigTriangle and drawSmallTriangle, but that would be a 
# # lot of redundant code.

# # The better way is to use arguments, also called parameters. 
# # (Those two terms are not perfectly synonymous, but they are often 
...
# # We've seen how to pass data into a function. 
# # What if you want to get data out of a function?
# # We'll demonstrate this by rebuilding the distance function. 
# # Let's start with an example of how distance works:
...
# # Now, let's imagine we have access to pos() but not distance(). 
# # We can make it ourselves like so:
...
# ## 19
# ## Make it so if you press "Up" on your keyboard, 
# ##the turtle will draw a square.

# ##Misc practice
# # Before continuing to the following exercises,
# # you should have completed the if, for, and while exercises.
